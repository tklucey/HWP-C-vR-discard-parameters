...
#Unlist df and make an array
  ...
  ppr_preArray <- left_join(pp_ratios2, harv_cf1, by = "Year", relationship = "many-to-many") %>%    #Combining data, creating column of the product of the Product Ratio and cubic feet of harvest
    ...
    arrange(Year, factor(Source, levels = ownership.names)) #added relationship = "many-to-many" to silence warnings
  ...
  eu_ratios <- eur %>%    # the resulting number of rows should be 175616 (yrs *  ownerships * number of End Use Products)
    ...
    left_join(ppr_preArray, by = c("TimberProductID", "PrimaryProductID" , "Year"), relationship = "many-to-many") %>% #added relationship = many-to-many here to silence warnings
...
###Added this 1/26/23  
###Doing the same for pulp wood as for fuelwood above: finding rows that correspond to pulp wood so that 8% loss not applied.
  # eur.pulp <- grep("pulp", ratio_cat$EndUseProduct) #moved up from code below.
  # 
  # pulp_array <- array(0, c(N.EUR, N.OWNERSHIP, N.YEARS))
  # pulp_array[eur.pulp, , ] <- eu_array[eur.pulp, , ]  # Grabbing the entries that correspond with pulp wood - this is the C into each of the end-use categories.
  # dimnames(pulp_array) <-  list(c(1:N.EUR), ownership.names, c(min(harv_cf$Year):max(harv_cf$Year)))
...
###EDITING HERE 117-153 - only thing still missing is making sure the 8% PIU.LOSS is not applied to paper
# Need the discarded products array (dp_array), generated every year from new timber harvest
  
  eur.pulp <- grep("pulp", ratio_cat$PrimaryProduct)      #End use ratio rows for wood pulp (100% used, no discard)
  ...
  #dp_array_new <- dp_array + pulp_array #NEW adding in 'pulp_array' for paper
...
#My new code starts here - not sure dp.total_array.df necessary anymore, but we'll see. I think ratio_cat2 is the only necessary part.
  ...
  ratio_cat2$EndUseID <- as.factor(ratio_cat2$EndUseID); ratio_cat2$DiscardID <- as.factor(ratio_cat2$DiscardID)
...
##discard.fates2 <- left_join(discard.fates2, disc.id[,c("DiscardID","Type")], by = c("DiscardType" = "Type"))
  
  #now DiscardType includes the additional prod categories
...
##Keep the hl code, but need to change the landfill.fx.ratio to something else
...
#assigning half-lives and fixed carbon ratios to discarded products by DiscardType
  ...
  landfill.fx.ratio <- left_join(disc.id, discard.hl[,c(1,3)]) %>% replace(is.na(.), 0)
  ...
  eu_ratios2 <- left_join(eu_ratios, dec.prop[,1:3], by = c("Year", "Type" = "DiscardType")) %>%
    left_join(burn.prop[,1:3], by = c("Year", "Type" = "DiscardType")) %>%
    left_join(recov.prop[,1:3], by = c("Year", "Type" = "DiscardType")) %>%
    left_join(compost.prop[,1:3], by = c("Year", "Type" = "DiscardType")) %>%
    left_join(landfill.prop[,1:3], by = c("Year", "Type" = "DiscardType")) %>%
    left_join(dumps.prop[,1:3], by = c("Year", "Type" = "DiscardType")) %>%
    replace(is.na(.), 0)
  #all various discard destinations now in same df
  ...
  lf.fixed_array <- array(landfill.fx.ratio$Landfills_fixed, c(N.EUR, N.OWNERSHIP, N.YEARS)) * landfill.input_array
  
  # The remaining carbon in the landfill available for decay:
  lf.available_array <- landfill.input_array  - lf.fixed_array
  
##NEWWWW 11/28/2023  ### Code just for methane estimates #### 
  # lf.available_df <- data.frame(lf.available_array) ##NEW
  # BLM.lf.avail <- lf.available_df[ , grepl( "BLM" , names( lf.available_df ) ) ]
  # PVT.lf.avail <- lf.available_df[ , grepl( "PVT" , names( lf.available_df ) ) ]
  # USFS.lf.avail <- lf.available_df[ , grepl( "USFS" , names( lf.available_df ) ) ]
  # Tribal.lf.avail <- lf.available_df[ , grepl( "Tribal" , names( lf.available_df ) ) ]
  # STATE.lf.avail <- lf.available_df[ , grepl( "STATE" , names( lf.available_df ) ) ]
  # Total.lf.avail <- lf.available_df[ , grepl( "Total" , names( lf.available_df ) ) ]
  # 
  # list_of_datasets <- list("BLM" = BLM.lf.avail, "PVT" = PVT.lf.avail, "State" = STATE.lf.avail, "Tribal" = Tribal.lf.avail,"USFS" = USFS.lf.avail, "Total" = Total.lf.avail)
  # write.xlsx(list_of_datasets, file = "C:/Users/keach/OneDrive - University of Massachusetts/GitHub/HWP_Shared_Test-main/HWP_Shared_Test-main/CA Landfill outputs_TEST\\list_of_datasets.xlsx")


  # Recovered (had to add a 3 instead of 2 for Decay1.fcn... decay.matrix[[k,3]])
  recov_decay <- Decay1.fcn(empty.eu_array, first.eu_array, recov.input_array, recov.hl, N.EUR, N.OWNERSHIP, N.YEARS) # The empty.eu_array or first.eu_array remain the same as earlier,
  ...
  landfill_decay <- Decay1.fcn(empty.eu_array, first.eu_array, lf.available_array, landfill.hl, N.EUR, N.OWNERSHIP, N.YEARS) 
  landfill_array <- landfill_decay[["decay"]]
  landfill.discard_array <- landfill_decay[["da"]]
  
  # Dumps
  dumps_decay <- Decay1.fcn(empty.eu_array, first.eu_array, dumps.input_array, dump.hl, N.EUR, N.OWNERSHIP, N.YEARS) 

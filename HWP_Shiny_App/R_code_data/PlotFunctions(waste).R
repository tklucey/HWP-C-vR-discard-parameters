...
#edited lines 142-143. Added ) after as.numeric(Year), and commented out the rest.
...
##Changes here to match HWP_Model_Function1 1/26/23 #NEW
  # eur.pulp <- grep("pulp", ratio_cat$PrimaryProduct)
  # 
  # # end use array for pulp #NEW
  # pulp_matrix <- matrix(0, nrow = N.EUR, ncol = d.yrs)                                
  # pulp_matrix[eur.pulp, ] <- eu_matrix[eur.pulp, ] 
...
####MAKING NEW EDITS TO CODE HERE 1/26/23
####separating out fuelwood and paper products so that the 8% loss is not applied to those categories.
...
####Commented out lines 271 - 286 from original code to see if it makes a difference with paper outputs.
####Another possibility here could be to run this using new paper categories one by one. Super annoying, but maybe it would work.
...
####Removed the 2 from the end of 'eu_ratios2'
  dec.input_matrix <- matrix(eu_ratios$DEC, nrow = N.EUR) * dp.total_matrix
  bwoec.input_matrix <- matrix(eu_ratios$BWoEC, nrow = N.EUR) * dp.total_matrix
  recov.input_matrix <- matrix(eu_ratios$Recovered, nrow = N.EUR)  * dp.total_matrix      
  compost.input_matrix <- matrix(eu_ratios$Composted, nrow = N.EUR)  * dp.total_matrix      
  landfill.input_matrix <- matrix(eu_ratios$Landfills, nrow = N.EUR) * dp.total_matrix      
  dumps.input_matrix <- matrix(eu_ratios$Dumps, nrow = N.EUR) * dp.total_matrix 
  ...
  ##NEWWWW 11/29/2023  ### Code just for methane estimates #### 
  # lf.available_df2 <- data.frame(lf.available_matrix) ##NEW
  # BLM.lf.avail2 <- lf.available_df2[ , grepl( "BLM" , names( lf.available_df2 ) ) ]
  # PVT.lf.avail2 <- lf.available_df2[ , grepl( "PVT" , names( lf.available_df2 ) ) ]
  # USFS.lf.avail2 <- lf.available_df2[ , grepl( "USFS" , names( lf.available_df2 ) ) ]
  # Tribal.lf.avail2 <- lf.available_df2[ , grepl( "Tribal" , names( lf.available_df2 ) ) ]
  # STATE.lf.avail2 <- lf.available_df2[ , grepl( "STATE" , names( lf.available_df2 ) ) ]
  # Total.lf.avail2 <- lf.available_df2[ , grepl( "Total" , names( lf.available_df2 ) ) ]
  # 
  # list_of_datasets2 <- list("BLM" = BLM.lf.avail2, "PVT" = PVT.lf.avail2, "State" = STATE.lf.avail2, "Tribal" = Tribal.lf.avail2,"USFS" = USFS.lf.avail2, "Total" = Total.lf.avail2)
  # write.xlsx(list_of_datasets2, file = "C:/Users/keach/OneDrive - University of Massachusetts/GitHub/HWP_Shared_Test-main/HWP_Shared_Test-main/CA Landfill outputs_TEST\\list_of_datasets2.xlsx")
...
### NEW CODE: Function to create the half lives for recovered material, landfill, and dumps
HLCreation.fcn <- function(discard.col, discard.halflives, pulp, n.eur, disc.id) {
  ...
  for(k in 1:N.EUR) {
    for(j in 1:N.OWNERSHIP) {
      ...
      for(i in first.array[k,j]:N.YEARS) {
        decay.array[k, j, i] <- as.numeric(target.array[k, j, i] + decay.array[k, j, i-1] * exp(-1 * log(2)/decay.matrix[[k,2]])) #changed '2' to 3 after decay.matrix[[k,]].
        #Had to change it back to 2 because it was out of bounds.
      }
    }
  }
  # Products in Use Discards (End Uses Totals -minus- Products in Use array )
...
###NEW CODE: ADDED ifelse() for other product categories
